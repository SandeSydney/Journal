...
class LoginBloc with Validators {
  ...
  void _startListenersIfEmailPasswordAreValid() {
    ...
    // login or create listener
    loginOrCreate.listen((action) {
      action == 'login' ? _logIn() : _createAccount();
    });
  }

  // check whether _emailValid and _passwordValid evaluate to true
  ...
  // login in user with email/ password credentials
  Future<String> _logIn() async {
    String _result = ''; // tracks if login is a success or failure
  ...
  // Create account for the user & login user if successful
